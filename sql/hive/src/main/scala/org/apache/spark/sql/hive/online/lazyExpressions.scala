...
case class Seed(reference: RelationReference) extends TransformableTag {
  val symbol = ":"
  def toTag = this
}

case class Bootstrap(branches: Seq[RelationReference]) extends TransformableTag {
  val symbol = ":"
  def toTag = this
  override lazy val simpleString = s"^${branches.mkString("[", ", ", "]")}"
}
...
case class ScaleFactor(branches: Seq[RelationReference]) extends LeafExpression {
...
case class GetPlaceholder(attribute: Attribute, children: Seq[Expression]) extends Expression {
  override type EvaluatedType = Any
