...
class OnlineDataFrame(dataFrame: DataFrame) extends org.apache.spark.Logging {
  ...
  def collectNext(): Array[Row] = {
    ...
    if (sparkPlanQueue.nonEmpty) {
      ...
      val dfStart = System.nanoTime()
      val df = sqlContext.createDataFrame(rdd, schema)
      val dfEnd = System.nanoTime()
      val executeTime = (executeEnd - executeStart) / 1000 / 1000
      val dfTime = (dfEnd - dfStart) / 1000 / 1000
      logInfo(s"LOGAN: execute time took ${executeTime}")
      logInfo(s"LOGAN: makeDF time took ${dfTime}")
  ...
  def next(): DataFrame = {
    ...
    val mb = 1024 * 1024
    logInfo(s"LOGAN: sanityCheck 3: ${runtime.freeMemory() / mb}")
    val x = makeSparkPlan(batches)
    logInfo(s"LOGAN: sanityCheck 4: ${runtime.freeMemory() / mb}")
    val rdd = x.execute()
