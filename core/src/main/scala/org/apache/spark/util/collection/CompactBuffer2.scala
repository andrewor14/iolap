 ...
 * so it has about 80-100 bytes of overhead. In contrast, CompactBuffer2 can keep up to two
 ...
 */
private[spark] class CompactBuffer2[T: ClassTag] extends Seq[T] with Serializable {
  var hasUncertain: Boolean = false
  ...
  def ++= (values: TraversableOnce[T]): CompactBuffer2[T] = {
    values match {
      // Optimize merging of CompactBuffer2s, used in cogroup and groupByKey
      case compactBuf: CompactBuffer2[T] =>
        val oldSize = curSize
  ...
  override def toString(): String = s"${iterator.mkString("[", ",", "]")}:$hasUncertain"
}

private[spark] object CompactBuffer2 {
  def apply[T: ClassTag](): CompactBuffer2[T] = new CompactBuffer2[T]

  def apply[T: ClassTag](value: T): CompactBuffer2[T] = {
    val buf = new CompactBuffer2[T]
    buf += value
  }
}
