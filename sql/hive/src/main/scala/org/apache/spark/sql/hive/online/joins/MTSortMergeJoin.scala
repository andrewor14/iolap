 ...
 */
@DeveloperApi
case class MTSortMergeJoin(
    leftCacheFilter: Option[Attribute],
    rightCacheFilter: Option[Attribute],
    leftRefresh: RefreshInfo,
    rightRefresh: RefreshInfo,
  ...
  extends BinaryNode with Stateful with IteratorRefresher {
  ...
  def retrieveState(): RDD[Row] = prevBatch match {
    case Some(bId) =>
      ...
      refresh(OLABlockRDD.create[Row](sparkContext, opId.id, Array((numParts, bId)), numParts))
    case None =>
      sys.error(s"Unexpected prevBatch = $prevBatch")
  }

  // Hacky: we do not need to consider non-cache side for now
  //  because we never cache anything on the non-cache side
  override val streamRefresh: RefreshInfo = cacheSide match {
    case CacheLeft => leftRefresh
    case CacheRight => rightRefresh
  }
  ...
  protected override def doExecute(): RDD[Row] = {
    val (leftResults, rightResults) = prevBatch match {
      ...
      case Some(_) =>
        cacheSide match {
          case CacheLeft =>
            if (left.execute().count() != 0) {
              controller.getWatcher += -1
              logError(s"Integrity Error in MTSortMergeJoin(Op $opId, Batch $currentBatch)")
            }
            (retrieveState(), right.execute().map(_.copy()))
          case CacheRight =>
            if (right.execute().count() != 0) {
              controller.getWatcher += -1
              logError(s"Integrity Error in MTSortMergeJoin(Op $opId, Batch $currentBatch)")
            }
            (left.execute().map(_.copy()), retrieveState())
        }
    }
  ...
  override def newBatch(newTrace: List[Int]): SparkPlan =
    MTSortMergeJoin(leftCacheFilter, rightCacheFilter, leftRefresh, rightRefresh,
      leftKeys, rightKeys, cacheSide, left, right)(controller, newTrace, opId)
}
